...
async function main() {
  try {
    // Manually construct JSON schema for the Carnet manifest
    // This represents the structure defined in src/lib/schemas.ts
    const jsonSchema = {
      $schema: 'http://json-schema.org/draft-07/schema#',
      title: 'Carnet Manifest Schema',
      description: 'Schema of the generated Carnet manifest',
      type: 'object',
      required: ['version', 'app', 'agents', 'skills', 'toolsets', 'tools'],
      properties: {
        version: {
          type: 'number',
        ...
        app: {
          ...
          required: ['globalInitialSkills', 'globalSkills'],
          properties: {
            globalInitialSkills: {
              type: 'array',
              items: {
            ...
            globalSkills: {
              ...
              description:
                'Skills available to all agents globally during their lifecycle, loaded on demand',
              default: [],
            },
          },
          ...
          additionalProperties: {
            ...
            properties: {
              ...
              skills: {
                ...
                description:
                  'Skills that can be loaded on demand by the agent during its lifecycle',
                default: [],
              },
              prompt: {
                type: 'string',
        ...
        toolsets: {
          ...
          additionalProperties: {
            ...
            required: ['name', 'description', 'tools', 'content'],
            properties: {
              name: {
                type: 'string',
    ...
    const markdown = `# Manifest Schema
...
`
    ...
    await fs.writeFile(path.join(docsDir, 'manifest-schema.md'), markdown.trim())

    console.log('✓ Generated manifest schema documentation')
  } catch (error) {
    console.error('Error generating manifest schema docs:', error)
    process.exit(1)
  }
}
