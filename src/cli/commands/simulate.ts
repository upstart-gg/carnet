	...
	if (diagnostics) {
		...
		console.log(
			`${colors.info('Requested Domain Tools:')} ${
				diagnostics.requestedDomainTools.length > 0
					? diagnostics.requestedDomainTools.join(', ')
					: '(none)'
			}`
		)
		console.log(
			`${colors.info('Filtered Tools:')} ${
				diagnostics.filteredTools.length > 0 ? diagnostics.filteredTools.join(', ') : '(none)'
			}`
		)
	}
...
async function loadSkill(carnet: Carnet, agentName: string, manifest: Manifest) {
	...
	const availableSkills = agent.skills.filter(
		(skill) => !session?.discoveredSkills.has(skill)
	)
